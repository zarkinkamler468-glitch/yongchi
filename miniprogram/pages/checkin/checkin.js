...
Page({
  ...
  submit() {
    ...
    request('/api/entries/preview?keyword=' + encodeURIComponent(keyword) + '&people=' + (Number(people) || 1)).then((d) => {
      this.setData({
        preview: { card_id: d.card.id, name: d.member.name, member_no: d.member.member_no, phone: d.member.phone || '', card_name: d.card.card_name || '', remaining_uses: d.card.remaining_uses, balance: fmtMoney(d.card.balance), end_at: d.card.end_at || '—', deduct_uses: d.card.preview_deducted_uses, deduct_amount: fmtMoney(d.card.preview_deducted_amount) }, result: null
      });
